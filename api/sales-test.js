  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (body.customerName && body.productId && body.premium) {
        const productData = getFirstEnrollDemoProducts().find(p => p.id === body.productId);
      ...
      const productData = getFirstEnrollDemoProducts().find(p => p.id === productId);
      if (!productData) {
        return res.status(404).json({ error: 'Product not found' });
      }
