...
async function syncHistoricalLeads(credentials, options = {}) {
    ...
    try {
        // In production, this would make actual API calls to Convoso
        // For development, we'll simulate the sync with mock data
        ...
        */
        ...
        // Mock historical leads for development
        const mockLeads = [];
        const leadsToGenerate = Math.min(limit, 25); // Simulate up to 25 leads per page
        
        for (let i = 0; i < leadsToGenerate; i++) {
            const leadId = `HIST_${agencyId}_${String(offset + i + 1).padStart(5, '0')}`;
            const campaigns = ['CAMP_AUTO_001', 'CAMP_HOME_002', 'CAMP_LIFE_003'];
            const selectedCampaign = campaignId || campaigns[Math.floor(Math.random() * campaigns.length)];
            
            mockLeads.push({
                lead_id: leadId,
                external_id: `EXT_${Math.random().toString(36).substr(2, 9)}`,
                phone_number: `555${Math.floor(Math.random() * 9000 + 1000)}`,
                first_name: ['John', 'Jane', 'Mike', 'Sarah', 'David', 'Lisa'][Math.floor(Math.random() * 6)],
                last_name: ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia'][Math.floor(Math.random() * 6)],
                email: `lead${offset + i + 1}@example.com`,
                source: 'convoso',
                campaign_id: selectedCampaign,
                campaign_name: selectedCampaign.replace('_', ' ').replace('CAMP', 'Campaign'),
                cost: Math.round((Math.random() * 50 + 15) * 100) / 100,
                state: ['CA', 'TX', 'FL', 'NY', 'IL'][Math.floor(Math.random() * 5)],
                city: 'Test City',
                zip_code: String(Math.floor(Math.random() * 90000 + 10000)),
                age: Math.floor(Math.random() * 40 + 25),
                insurance_type: selectedCampaign.includes('AUTO') ? 'auto' : 
                              selectedCampaign.includes('HOME') ? 'home' : 
                              selectedCampaign.includes('LIFE') ? 'life' : 'auto',
                lead_score: Math.floor(Math.random() * 40 + 50),
                status: 'new',
                priority: Math.random() > 0.8 ? 'high' : 'normal',
                received_at: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(), // Random time within last week
                additional_data: {
                    sync_source: 'historical',
        ...
        return {
            ...
            nextOffset: offset + limit < 500 ? offset + limit : null
        };
