...
export function isDemoUser(email, agencyId) {
  return false; // Demo functionality removed
}

/**
 * Apply production data filtering to Supabase queries
 * @deprecated Demo functionality removed - this now only returns production data
 */
export function applyDataIsolation(query, tableName, isDemo = false, filterField = 'sale_id') {
  // Always return production data (exclude any demo patterns that might exist)
  switch (tableName) {
...
/**
 * Get lead summary data with proper empty state handling
 */
