// Customer Service Portal Global Header Component
// Provides consistent header across all customer service pages
...
(function() {
    ...
    const CSHeader = {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (userData.firstName && userData.lastName) {
                        return {
                            role: userData.role || 'customer-service',
                ...
                if (typeof SyncedUpAuth !== 'undefined' && SyncedUpAuth.getCurrentUser) {
                    ...
                    if (user) {
                        ...
                        return {
                            ...user,
                            name: userName,
                            role: user.role || 'customer-service'
                        };
                    }
                }
                ...
                return {
                    role: userRole || 'customer-service',
                    name: userName || userEmail || 'Customer Service',
                    email: userEmail || ''
                };
            } catch (error) {
        ...
        buildHeader: function(userData) {
            const userDisplay = userData ? `${userData.name || 'Customer Service'} (${userData.role || 'CS'})` : 'Loading...';
            
            let headerContent = `
                <div class="${this.config.containerClass}">
                    <h1>${this.config.pageTitle}</h1>
                    <div class="user-info" style="display: flex; justify-content: center; align-items: center; gap: 1rem; padding: 1rem 0;">
            `;
            ...
            // Add logout button
            headerContent += `
                        <button class="btn btn-secondary" onclick="CSHeader.logout()">
        ...
        initializeComponents: function(userData) {
            ...
            if (this.config.showRoleSwitcher && window.RoleSwitcher) {
                // CS agents might be able to switch to other roles
                ...
                if (userRoles) {
                    ...
                    // CS can switch to other roles they have
            ...
            // Load navigation
            if (window.CSNavigation && window.CSNavigation.render) {
                window.CSNavigation.render();
            } else {
        ...
        refreshUserData: async function() {
            const userData = await this.getUserData();
            const displayText = userData ? `${userData.name || 'Customer Service'} (${userData.role || 'CS'})` : 'Loading...';
