...
async function quoteHandler(req, res) {
  ...
  try {
    const { agencyId, agentId } = getUserContext(req);
    ...
    if (!products || products.length === 0) {
      selectedProducts = getFirstEnrollDemoProducts().filter(p => 
    ...
    const quotes = selectedProducts.map(product => {
      ...
      return {
        ...
        effectiveDate: quoteOptions?.effectiveDate || getNextMonthFirst(),
    ...
    const quoteRecord = {
      ...
      products_quoted: quotes,
      quote_status: 'active',
      created_at: new Date().toISOString(),
      expires_at: getQuoteExpirationDate()
    };

    // Try to save to database, but don't fail if table doesn't exist
    try {
      await supabase.from('quotes').insert(quoteRecord);
    } catch (dbError) {
      console.log('Could not save quote to database (table may not exist):', dbError.message);
    }

    // Return the quotes
    res.status(200).json({
      quoteNumber: quoteRecord.id,
      quotes,
      customerInfo,
      agentInfo: {
        agentId,
...
function calculateProductQuote(product, customerInfo, coverageOptions = {}) {
  let basePremium = product.premium;
  let discounts = [];
  ...
  // State-based adjustments (example)
  ...
  const finalMonthlyPremium = Math.max(adjustedPremium - totalDiscounts, basePremium * 0.7); // Minimum 70% of base
  const annualPremium = finalMonthlyPremium * 12;
  const commissionAmount = annualPremium * (product.commission_rate / 100);
...
function generateBenefitsList(product) {
  ...
  if (product.deductible <= 1000) {
    benefits.push('Low deductible plan');
  }
  
  if (product.product_type === 'health' && product.copay_primary <= 25) {
