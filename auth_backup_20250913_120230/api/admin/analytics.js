...
async function getOverviewAnalytics() {
  try {
    ...
    // Get sales/revenue from portal_sales
    const { data: sales, error: salesError } = await supabase
      .from('portal_sales')
      .select('premium')
      .eq('status', 'completed');

    const totalRevenue = sales?.reduce((sum, s) => sum + (parseFloat(s.premium) || 0), 0) || 0;

    // Calculate conversion rate (leads to sales)
    let conversionRate = 0;
    if (totalLeads > 0 && sales?.length > 0) {
      conversionRate = (sales.length / totalLeads * 100).toFixed(1);
    }
